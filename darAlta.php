...
<?php include('includes/metaAndCSS.html'); ?>
  ...
  <div id='page-wrap'>
	...
	<div>
		...
		<input type="button" class="botonesgestion" name="alta" value="Dar de alta" onclick="darAltaAJAX()"/>
		<div id="mensaje"></div>
	</div>
    </section>
...
<script>
	...
	function mostrarUsuariosAJAX(){
		...
		$.ajax({
			url: 'AJAX/selectUsuariosAlta.php',
			type: "POST",
	...
	function darAltaAJAX(){
		
		var email = $("#email").val();
	
		$.ajax({
			url: 'AJAX/darAltaAJAX.php',
			type: "POST",
			data: "email="+email,
			...
			console.log(datos);
				if(datos=="OK")
					$('#mensaje').html('<p class="error"><strong>Usuario dado de alta</p>');
				else
					$('#mensaje').html('<p class="error"><strong>No se ha podido dar de alta el usuario</p>');	
					},
			error:function(){
			$('#mensaje').fadeIn().html('<p class="error"><strong>No se ha podido dar de alta el usuario</p>');
			}
		});
	}	
</script>
